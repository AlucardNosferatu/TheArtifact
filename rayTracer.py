...
def screen_map_function(pos):
    ...
    return (
    int((pos[0] - coord_lims[0][0]) / rangeX * size[0]), int((-pos[1] - coord_lims[1][0]) / rangeY * size[1]), scaleX,
    scaleY)
...
def screen_map_inv(pos_screen):
    ...
    scaleY = float(size[1]) / rangeY
    return (
        int((pos_screen[0] / size[0] * rangeX + coord_lims[0][0])), int(-pos_screen[1] * rangeY / size[1] - coord_lims[1][0]),
        scaleX, scaleY)
